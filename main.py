...
async def synthesize_speech(request: TTSRequest):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            logger.error(f"VibeVoice Output: {result.stdout}")
            raise HTTPException(status_code=500, detail=f"VibeVoice Fehler: {result.stderr[:500]}")
        
        logger.info(f"VibeVoice Output: {result.stdout}")
        
        # Finde generierte Audio-Datei (VibeVoice speichert in outputs/)
        possible_dirs = [
            Path(f"{VIBEVOICE_DIR}/demo/outputs"),
            Path(f"{VIBEVOICE_DIR}/outputs"),
            Path("outputs")
        ]
        
        output_path = None
        for output_dir_path in possible_dirs:
            if output_dir_path.exists():
                logger.info(f"Prüfe Verzeichnis: {output_dir_path}")
                output_files = sorted(output_dir_path.glob("*.wav"), key=lambda x: x.stat().st_mtime, reverse=True)
                if output_files:
                    output_path = str(output_files[0])
                    logger.info(f"Audio gefunden in {output_dir_path}: {output_path}")
                    break
        
        if not output_path:
            # Suche in der gesamten stdout nach dem Pfad
            for line in result.stdout.split('\n'):
                if '.wav' in line and 'save' in line.lower():
                    logger.info(f"Gefunden in stdout: {line}")
            
            raise HTTPException(status_code=500, detail="Keine Audio-Datei generiert. Prüfe Logs.")
